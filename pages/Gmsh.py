...
if choice == "Общая характеристика ПО":
    ...
    st.write("""**Количество под-ребер (Geometry.NumSubEdges)**""")
    st.write("""
    ...
    # Введение
    st.write("""
    Для генерации поверхностной сетки в Gmsh предлагаются три основных алгоритма: **MeshAdapt**, **Delaunay** и **Frontal-Delaunay**.
    """)
    ...
    st.title("Генерация 2D-сеток на прямоугольнике с использованием Gmsh")
    ...
    # Заголовок страницы
    st.title("Различия между методом Delaunay и методом Frontal")

    # Введение
    st.write("""
    Методы **Delaunay** и **Frontal** широко используются для генерации сеток.  
    Они отличаются принципами работы, скоростью, качеством элементов и областью применения.
    ...
    if aspect == "Принцип работы":
        ...
        st.write("""
        - **Метод Delaunay**:  
        Основан на триангуляции Delaunay, где каждый треугольник удовлетворяет условию:  
        внутри описанной окружности нет других точек сетки.  
        Это обеспечивает равномерность и высокое качество элементов.
        
        - **Метод Frontal**:  
        Комбинирует фронтальный метод с триангуляцией Delaunay.  
        Начинается с границ объекта и постепенно добавляет новые точки,  
        поддерживая условие Delaunay для каждого нового элемента.
        """)
    ...
    # Качество элементов
        ...
        - **Метод Frontal**:  
        Генерирует почти равносторонние треугольники, минимизируя искажения.  
        Это особенно важно в задачах с высокими требованиями к точности.
        """)
    ...
    st.write("""
    В целом, метод Delaunay быстрее и более устойчив, но может уступать Frontal  
    по качеству элементов. Выбор метода зависит от требований к точности и сложности геометрии.  
    """)
